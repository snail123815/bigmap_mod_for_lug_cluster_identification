...
from pyBioinfo_modules.wrappers.antismash import (clusterGbkGlobTxt,
                                                  findClusterNumberStr)
from pyBioinfo_modules.wrappers.bigscape import runBigscape
from pyBioinfo_modules.wrappers.mash import (calculate_medoid, mashDistance,
                                             mashSketchFiles)
...
def main():
    ...
    try:
        if gbksListFile.exists():
            ...
            for d in tqdm(
                [d for d in inputPath.iterdir() if d.is_dir()],
                desc=(f"Gathering gbk files from {inputPath.name}"),
            ):
                gbks.extend(
                    Path(gbk)
                    for gbk in glob(
                        str(d / ("**/" + clusterGbkGlobTxt)), recursive=True
                    )
                )
